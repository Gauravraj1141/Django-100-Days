...
from .models import PropertyDetail, City
    ...
    if request.method == "POST":
        ...
        if formdata.is_valid():
            ...
            PropertyDetail.objects.create(
                Property_name=propertyname, Property_address=propertyaddress, City_name=propertycity)
...
def Fetch_Property(request):
    if request.method == "POST":
        ...
        properties = PropertyDetail.objects.filter(
            City_name__ctname__iexact=cityname)
...
def Find_cities(request):
    if request.method == "POST":
        ...
        cities = PropertyDetail.objects.filter(
            City_name__state__stname__iexact=statename)
...
def Find_similiar_pr(request):
    if request.method == "POST":
        ...
        similiar_data = PropertyDetail.objects.filter(
            City_name=cityname)
